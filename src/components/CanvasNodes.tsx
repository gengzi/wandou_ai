...
const EmptyState = ({ children }: { children: React.ReactNode }) => (
  <div className="max-h-28 overflow-auto rounded-xl border border-dashed border-white/10 bg-[#1A1A1C]/70 p-4 text-center text-[12px] leading-5 text-slate-500 [overflow-wrap:anywhere] whitespace-pre-wrap">
    {children}
  </div>
);
...
export const ScriptNode = ({ id, data }: any) => {
  ...
  const summary = getDisplayText(data?.output?.summary, '等待智能体流程生成剧本摘要。');
    ...
    <div className="w-[420px] bg-[#111112] rounded-2xl border border-white/10 shadow-2xl overflow-hidden cursor-default group hover:border-white/20 transition-colors">
      ...
      <div className="bg-[#1A1A1C] px-4 py-3 flex items-center justify-between border-b border-white/5">
        <div className="flex min-w-0 items-center space-x-3">
          ...
          <span className="min-w-0 truncate text-xs font-bold text-slate-300">{title}</span>
          <span className={`text-[10px] px-1.5 py-0.5 rounded border ${status === 'success' ? 'text-brand border-brand/30 bg-brand/10' : status === 'running' ? 'text-yellow-300 border-yellow-300/20 bg-yellow-300/10' : 'text-slate-500 border-white/10 bg-white/5'}`}>{statusLabel(status)}</span>
          <span className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[10px] text-slate-400">角色一致性模型</span>
        </div>
        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => emitNodeAction('quote', id, data)} className="w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 text-slate-400 hover:text-brand transition-colors" title="引入到对话">
            <CopyPlus size={14} />
           ...
           {imageSrc ? (
             <>
               {videoUrl ? (
                 ...
                 <>
                   <img src={imageSrc} className="w-full h-full object-cover" alt="节点媒体" />
    ...
    <div className="w-[340px] bg-[#111112] p-4 rounded-2xl border border-white/10 shadow-2xl group relative hover:border-brand/40 transition-all">
      ...
      {imageUrls.length > 0 ? (
        ...
        <div className="space-y-3">
          {prompt && (
            <div className="max-h-28 overflow-auto rounded-xl border border-white/5 bg-[#1A1A1C] p-3 text-[12px] leading-5 text-slate-300 [overflow-wrap:anywhere] whitespace-pre-wrap">
              {prompt}
            </div>
          )}
  ...
  return (
    <div className="w-[300px] bg-[#111112] p-4 rounded-2xl border border-white/10 shadow-2xl group relative hover:border-brand/40 transition-all">
      ...
      <div className="bg-[#1A1A1C] p-3 rounded-xl border border-white/5 space-y-3">
         <p className="max-h-32 overflow-auto text-[11px] leading-5 text-slate-400 [overflow-wrap:anywhere] whitespace-pre-wrap">{prompt ? `提示词：${prompt}` : '等待后端返回声音设计。'}</p>
         {audioUrl ? <div className="h-10 bg-slate-900 border border-white/5 rounded-lg flex items-center px-3 space-x-3 group/audio cursor-pointer hover:border-brand/50 transition-colors">
            ...
            <span className="text-[9px] text-slate-500 font-mono">{duration || '音频'}</span>
         </div> : null}
      </div>
            ...
            <>
              {videoUrl ? (
                ...
                <div className="relative h-full w-full">
                  <img src={withAuthQuery(thumbnail)} className="h-full w-full object-cover" alt="成片预览" />
